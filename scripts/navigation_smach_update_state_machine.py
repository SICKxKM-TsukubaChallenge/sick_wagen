...
class StartWaiting(smach.State):
    def __init__(self):
        smach.State.__init__(self, outcomes=['start_navigation', 'error'])
        self.trigger_received = False
        self.srv = rospy.Service('start_navigation_trigger', Empty, self.handle_trigger)

    def handle_trigger(self, req):
        rospy.loginfo("Start navigation service called. Starting navigation...")
        self.trigger_received = True
        return EmptyResponse()

    def execute(self, userdata):
        rospy.loginfo("Waiting for external trigger to start navigation...")
...
def main():
    ...
    with sm:
        print("hogehoge")
    with sm:
        smach.StateMachine.add('START_WAITING', StartWaiting(),
                            transitions={'start_navigation': 'GOTO_WAYPOINT',
                                            'error': 'failure'})
