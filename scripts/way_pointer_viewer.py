...
while not rospy.is_shutdown():
	...
	with open(ROS_WAYPOINT_FILE, 'r') as f:
		counter = 0
		reader = csv.reader(f)
		header = next(reader)
		...
		for row in reader:
			...
			marker_data.pose.position.x = list(map(float,row)[1])
			marker_data.pose.position.y = list(map(float,row)[2])
			marker_data.pose.position.z = list(map(float,row)[3])

			marker_data.pose.orientation.x=list(map(float,row)[4])
			marker_data.pose.orientation.y=list(map(float,row)[5])
			marker_data.pose.orientation.z=list(map(float,row)[6])
			marker_data.pose.orientation.w=list(map(float,row)[7])
